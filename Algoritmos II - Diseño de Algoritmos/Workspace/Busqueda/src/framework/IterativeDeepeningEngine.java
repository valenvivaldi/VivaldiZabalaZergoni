...
public class IterativeDeepeningEngine<S extends State> extends AbstractSearchEngine<S> {
    ...
    public boolean performSearch() {
		...
		// first, we initialize the data structures for the search
        ...
        while(i<=max&&(!resultSearch)){
        	visited = new LinkedList<S>();
        	
        	resultSearch = recursiveDepthFirst(initialState,i);
        	i+=paso;
        };
     ...
     * successfull state is found.	 
	 */	
	private boolean recursiveDepthFirst(S s,int n) {
		if(n<0){return false;}
		...
		if (!visited.contains(s)) {
            ...
            if (problem.success(s)) {
				...
				while ( (!succ_s.isEmpty()) && (!found) )  {
					...
					succ_s.remove(0);
					found = recursiveDepthFirst(child,(n-1));
